...
import { OTPPlayerProps, DefaultPlayers } from "../../types/OTPTypes";
import { BroadcastWebsocket } from "../../types/WebsocketTypes";

let ws: BroadcastWebsocket<OTPPlayerProps[]>;

function OTPRace() {
  const [otpRaceData, setOTPRaceData] = useState<OTPPlayerProps[]>(DefaultPlayers);

  const { accountName } = useParams();

  useEffect(() => {
    if (!ws && accountName) {
      ws = new BroadcastWebsocket<OTPPlayerProps[]>(accountName, setOTPRaceData);
    }
  }, []);
  ...
  const karmaIndex = otpRaceData.findIndex((p) => p.summonerName === "Fat Houdini")!;
  const betterKarmaIndex = otpRaceData.findIndex(
    (p) => p.combinedLP >= otpRaceData[karmaIndex].combinedLP && p.summonerName !== "Fat Houdini"
  )!;
  ...
  if (karmaIndex === bestOTP) {
    ...
    kontrahenten = [otpRaceData[karmaIndex - 1], otpRaceData[karmaIndex], otpRaceData[bestOTP]];
    kontrahenten[0].rang = 49 - karmaIndex - 1;
    kontrahenten[1].rang = 49 - karmaIndex;
    kontrahenten[1].rang = 49 - bestOTP;
  } else {
    console.log("Karma is third to best or worse");
    kontrahenten = [otpRaceData[karmaIndex], otpRaceData[betterKarmaIndex], otpRaceData[bestOTP]];
